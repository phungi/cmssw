// -*- C++ -*-
//
// Package:    RhoAnalyser/RhoAnalysis
// Class:      RhoAnalysis
//
/**\class RhoAnalysis RhoAnalysis.cc RhoAnalyser/RhoAnalysis/plugins/RhoAnalysis.cc
...
class RhoAnalysis : public edm::one::EDAnalyzer<edm::one::SharedResources> {
public:
  explicit RhoAnalysis(const edm::ParameterSet& iConfig);
  ~RhoAnalysis() override;

  static void fillDescriptions(edm::ConfigurationDescriptions& descriptions);
  // void fillDescriptionsFromRhoAnalysis(edm::ParameterSetDescription& desc);
...
//
RhoAnalysis::RhoAnalysis(const edm::ParameterSet& iConfig)
  :
...
void RhoAnalysis::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
...
void RhoAnalysis::endJob() {
  // please remove this method if not needed
}


void RhoAnalysis::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
// ------------ method fills 'descriptions' with the allowed parameters for the module  ------------
  ...
  // descriptions.add("RhoAnalysis", desc);
  descriptions.addWithDefaultLabel(desc);
}


// void RhoAnalysis::DefineRandomCone(const std::vector<pat::PackedCandidate> pfCandidateColl){
...
// void RhoAnalysis::RhoPhiModulation(double object_phi, double minFlowChi2Prob_, double maxFlowChi2Prob_, edm::Handle<std::vector<double> >  rhoFlowFitParams, double &rhoModulationFactor){
...
// void RhoAnalysis::ConeAxisModulation(const edm::Handle<std::vector<double> > rhoFlowFitParams, const bool useModulatedRho_ ){
...
// double RhoAnalysis::getModulatedRhoFactor(double phi, edm::Handle<std::vector<double>> flowComponents) {
//   //the function is a protected member of CSJetProducer, not sure if it's better to make RhoAnalysis inherit from that class, or this is OK?
...
// void RhoAnalysis::coneSegmentCorrection(edm::Handle<std::vector<double>> rhoFlowFitParams){
