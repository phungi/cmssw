...
process.source = cms.Source("PoolSource",
    ...
    fileNames = cms.untracked.vstring(
       ...
       '/store/group/phys_heavyions/santanas/UPC_dijet/samples/ccbar_MINIAODSIM_test2024.root'
    )
)
...
process.TFileService = cms.Service("TFileService",
    fileName = cms.string("HiForestMiniAOD_patjetsunsubtracted_stableonly2.root"))
...
process.load('HeavyIonsAnalysis.EventAnalysis.HiMergedGenAnalyser_cfi')

# process.HiGenParticleAna.stableOnly = cms.untracked.bool(False)
# process.HiGenParticleAna.genParticleSrc = cms.InputTag("mergedGenParticles")
# process.HiGenParticleAna.signalGenParticleSrc = cms.InputTag("MergedGenParticleProducer")
#####################################################################################
...
for jetLabel in jetLabels:
    ...
    getattr(process,"ak"+jetLabel+"PFJetAnalyzer").jetTag = "patJetsAK"+jetLabel+"PFUnsubJets"
    getattr(process,"ak"+jetLabel+"PFJetAnalyzer").jetName = 'ak'+jetLabel+'PF'
    getattr(process,"ak"+jetLabel+"PFJetAnalyzer").isMC = True
    
    # getattr(process,"ak"+jetLabel+"PFJetAnalyzer").genParticles = 'prunedGenParticles' //not reco::Candidates
    getattr(process,"ak"+jetLabel+"PFJetAnalyzer").genParticles = 'genParticlesForJets'
