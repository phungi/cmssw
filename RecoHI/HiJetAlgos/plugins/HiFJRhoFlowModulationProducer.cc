...
namespace {
  ...
  double weightFunction(double* x, double* par){
    ...
    if(TMath::Abs(x[1]) > par[0]){
      ...
      if(TMath::Abs(x[1]) - exclusionArea > par[0]) return 0;
...
HiFJRhoFlowModulationProducer::HiFJRhoFlowModulationProducer(const edm::ParameterSet& iConfig)
  ...
  if(firstFittedVn_ < 1) firstFittedVn_ = 2;
  if(lastFittedVn_ < 1) lastFittedVn_ = 2;
...
void HiFJRhoFlowModulationProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  const int nFlow = lastFittedVn_ - firstFittedVn_ + 1; // Number of flow components in the fit to particle flow condidate distribution
  double eventPlane[nFlow];
  for(int iFlow = 0; iFlow < nFlow; iFlow++) eventPlane[iFlow] = -100;

  // Initialize the output vector with flow fit components
  const int nParamVals = nFlow*2+4; // v_n and Psi_n for each fitted flow component, plus overall normalization factor, chi^2 and ndf for flow fit, and the first fitted flow component
  ...
  double eventPlaneCos[nFlow] = {0};
  double eventPlaneSin[nFlow] = {0};
  ...
  for (auto const& pfCandidate : pfCands) {
    ...
    if(std::abs(pfCandidate.eta()) > pfCandidateEtaCut_) continue;
    if(pfCandidate.pt() < pfCandidateMinPtCut_) continue;
    if(pfCandidate.pt() > pfCandidateMaxPtCut_) continue;
    ...
    if (doJettyExclusion_) {
      ...
      for (auto const& jet : *jets) {
        if (deltaR2(jet, pfCandidate) < 0.16) {
          ...
          if(deltaPhiJetPf > TMath::Pi()) deltaPhiJetPf = TMath::Pi()*2 - deltaPhiJetPf;
          thisPfCandidateWeight += pfWeightFunction_->Eval(deltaPhiJetPf, jet.eta()); // Weight currently does not take into account overlapping jets
        }
      }
